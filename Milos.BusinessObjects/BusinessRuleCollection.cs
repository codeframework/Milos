...
namespace Milos.BusinessObjects
{
    ...
    /// </summary>
    public class BusinessRuleCollection : List<IBusinessRule>
    {
        /// <summary>Constructor</summary>
        /// <param name="parentBusinessObject">Business object this collection lives in.</param>
        public BusinessRuleCollection(IBusinessObject parentBusinessObject) => BusinessObject = parentBusinessObject;

        /// <summary>Internal reference to the business object this collection lives in.</summary>
        protected IBusinessObject BusinessObject { get; }
        ...
        public void ApplyRules(DataSet currentDataSet, Type ruleType)
        {
            ...
            for (var iTableCounter = 0; iTableCounter < iMaxTables; iTableCounter++)
                // Then, we iterate over all our rules and see if they need to be applied to that particular table
            for (var iCounter = 0; iCounter < Count; iCounter++)
                if (this[iCounter].TableName.ToLower(CultureInfo.InvariantCulture) == currentDataSet.Tables[iTableCounter].TableName.ToLower(CultureInfo.InvariantCulture))
                    for (var iRowCounter = 0; iRowCounter < currentDataSet.Tables[iTableCounter].Rows.Count; iRowCounter++)
                        if (currentDataSet.Tables[iTableCounter].Rows[iRowCounter].RowState != DataRowState.Deleted)
                            if (this[iCounter].GetType() == ruleType)
                                this[iCounter].VerifyRow(currentDataSet.Tables[iTableCounter].Rows[iRowCounter], iRowCounter);
