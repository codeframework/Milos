...
public abstract class BusinessRule(string tableName, RuleViolationType ruleType = RuleViolationType.Violation) : IBusinessRule
{
    ...
    protected virtual void LogBusinessRuleViolation(DataRow currentDataRow, int rowIndex, string fieldName, string message) => BusinessObject.LogBusinessRuleViolation(currentDataRow.Table.DataSet, currentDataRow.Table.TableName, fieldName, rowIndex, violationType, message, GetType().FullName);
}
