...
public class NameAddressCollection(IBusinessEntity parentEntity) : EntityXlinkSubItemCollection(parentEntity), INameAddressCollection
{
    /// <summary>
...
public class NameAddressEntity(IEntitySubItemCollection parentCollection) : EntitySubItemCollectionXLinkItem(parentCollection), INameAddressEntity
{
    ...
    public string GetFormattedAddress(bool includeName)
    {
        ...
        if (includeName)
        {
            if (!string.IsNullOrEmpty(AddressCompany)) address += AddressCompany.Trim() + Environment.NewLine;
            if (!string.IsNullOrEmpty(AddressName)) address += AddressName.Trim() + Environment.NewLine;
        }

        address += Street.Trim() + Environment.NewLine;
        if (!string.IsNullOrEmpty(Street2)) address += Street2.Trim() + Environment.NewLine;
        if (!string.IsNullOrEmpty(Street3)) address += Street3.Trim() + Environment.NewLine;
        switch (entCountry.AddressFormat)
        {
            case AddressFormat.CityStateZip:
                address += City.Trim() + ", " + State.Trim() + " " + Zip.Trim() + Environment.NewLine;
                break;
            case AddressFormat.CityPostalCode:
                address += City.Trim() + ", " + Zip.Trim() + Environment.NewLine;
                break;
            case AddressFormat.PostalCodeCity:
                address += Zip.Trim() + " " + City.Trim() + Environment.NewLine;
                break;
            case AddressFormat.PostalCodeCityState:
                address += Zip.Trim() + " " + City.Trim() + ", " + State.Trim() + Environment.NewLine;
                break;
        }
